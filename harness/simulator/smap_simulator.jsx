...
function routeStep(pts, tgt, l, kEff, salt, bugDFixed) {
  ...
  for (const mv of moves) {
    ...
    // Bug D active-incumbent guard: if bug D is fixed, or if kEff === 1.0 (L63 global gate protects), the guard holds. 
    // If bug D is NOT fixed and kEff < 1.0, the guard is removed, allowing nearer inactive points to uncover active ones.
    const incumbentActive = pts[mv.nr][mv.nc].m > OFF_THRESH;
    const isSelf = mv.nr === mv.r && mv.nc === mv.c;
    const isProtected = bugDFixed || kEff >= 0.999;
    if (incumbentActive && !isSelf && isProtected) continue; // occlusion protected
    taken[key] = true;
...
function oneStep(pts, tgt, rectify, futFilterFixed, kEff, bFaithful, lr, salt, freezeFinest = false, schema = "MASK_OPT", bugDFixed = false) {
  let cur = pts.map(row => row.map(p => ({ ...p, stale: false })));
  for (let l = L_MAX; l >= (freezeFinest ? 1 : 0); l--) cur = routeStep(cur, tgt, l, kEff, salt, bugDFixed);
...
function routeOnly(pts, tgt, kEff, salt, freezeFinest = false, bugDFixed = false) {
  let cur = pts.map(row => row.map(p => ({ ...p, stale: false })));
  for (let l = L_MAX; l >= (freezeFinest ? 1 : 0); l--) cur = routeStep(cur, tgt, l, kEff, salt, bugDFixed);
  return cur;
}
...
export default function SMapSimulator() {
  ...
  const [bugDFixed, setBugDFixed] = useState(false);            // Bug D (mask-fallback value no-op)
  ...
  const nextPts = useMemo(() => oneStep(pts, tgt, rectify, futFilterFixed, kEff, bFaithful, lr, step + 1, freezeFinest, schema, bugDFixed),
    [pts, tgt, rectify, futFilterFixed, kEff, bFaithful, lr, step, freezeFinest, schema, bugDFixed]);
  ...
  const forward = () => commitPts(routeOnly(pts, tgt, kEff, step + 1, freezeFinest, bugDFixed), false);
  ...
  const loadSnapshot = (jsonStr) => {
    try {
      ...
      if (state.bugDFixed !== undefined) setBugDFixed(state.bugDFixed);
  ...
  const bugRows = [
    ...
    { id: "bugD", label: "Bug D: z-buffer mask-fallback", applies: true, fixed: bugDFixed,
      toggle: () => setBugDFixed(v => !v), severity: "SOURCE BUG", where: "smap.py L188 clobbers weights4",
      effect: bugDFixed ? "FIX (if applied): keeps center gather for fallback. Occlusion is unconditionally mask-preserving." : "SOURCE (buggy): L188 replaces center with winner, L189 is a value no-op. A nearer inactive winner uncovers a matched active cell (unless k=1.0 via L63 global gate)." },
