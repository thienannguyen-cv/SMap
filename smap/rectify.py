...
class DefaultRectify(torch.nn.Module):
    ...
    def compute_pre_allow_matrices(self, weight, target_repr):
        ...
        pre_allow[:,:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)] = (weight[:,:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)])
        self.smap3x3.vtestcase.testbot_input(pre_allow.reshape(BATCH_SIZE,-1,3*3,height, width)[:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)], filename="_comp_1_pre_Y.npy", dim=4)
        pre_allow[:,:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)] = (pre_allow[:,:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)])*target_repr.reshape(BATCH_SIZE,-1,1,1,h_zoom, w_zoom)
        ...
        self.smap3x3.vtestcase.testbot_input(pre_allow_s.reshape(BATCH_SIZE,-1,3*3,height, width)[:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)], filename="_comp_2_pre_Y_NOT_FOLD_OR_UNFOLDED_YET.npy", dim=4)
    ...
    def prepare_flows_for_coord(self, pre_allow, weights_b, target_repr):
        ...
        self.smap3x3.vtestcase.testbot_input(pre_allow.reshape(BATCH_SIZE,-1,3*3,height, width)[:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)], filename="_comp_2_pre_Y.npy", dim=4)
        allow = torch.max(pre_allow,dim=2,keepdim=True).values
        allow = 1.-allow
        ...
        allow = torch.cat([allow, allow, allow],dim=2)
        allow = torch.cat([allow, allow, allow],dim=3).reshape(BATCH_SIZE,-1,3,3,1,height, width)
        self.smap3x3.vtestcase.testbot_input(allow.reshape(BATCH_SIZE,-1,3*3,height, width)[:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)], filename="_Y_NOT_FOLD_OR_UNFOLDED_YET.npy", dim=4)
    ...
    def rectificate_flow(self, new_x_z_mask_value, pre_x, pre_y, pre_z, pre_mask, panels, target, original_size):
        ...
        C_zoom = new_x_z_mask_value.shape[1]  # [BUG 4 FIX]
        ...
        new_r_mask_values = new_r_mask.reshape(BATCH_SIZE,C_zoom,-1,height, width)
        new_r_mask = torch.zeros_like(weights_b.reshape(BATCH_SIZE,C_zoom,3*3,height, width))+new_r_mask_values
        ...
        pre_weights = utils.agg((new_x_z_mask_value[:,:,:,:,-1:,:,:]).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width)).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width)  # [BUG 4b FIX]
...
class CAMRectify(DefaultRectify):
    ...
    def rectificate_flow(self, new_x_z_mask_value, pre_x, pre_y, pre_z, pre_mask, panels, target, original_size):
        ...
        # testing/in
        pre_key_query = self.smap3x3.vtestcase.testbot_in(pre_key_query.reshape(BATCH_SIZE,C_zoom*3*3,height, width))
        ...
        # self.vtestcase.testbot_input((pre_mask.reshape(BATCH_SIZE,C_zoom*3*3,height, width)[:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)]).reshape(BATCH_SIZE,C_zoom,3*3,h_zoom, w_zoom)[:,:,4,:,:])
        ...
        allow = self.compute_pre_allow_matrices((pre_weights>specials.OFF_THRESH).float().detach().reshape(BATCH_SIZE,-1,3,3,1,height, width), target_2Dr)
        ...
        # testing/out
        pre_key_query = self.smap3x3.vtestcase.testbot_out(pre_key_query)
        ...
        coord_flow = self.prepare_flows_for_coord(allow, (weights_b>specials.OFF_THRESH).float().detach().reshape(BATCH_SIZE,-1,3,3,1,height, width), target_2Dr)
        mask_flow = self.prepare_flows_for_mask(allow, (weights_b>specials.OFF_THRESH).float().detach().reshape(BATCH_SIZE,-1,3,3,1,height, width), target_2Dr)
        target_2Dr = target_2Dr.reshape(BATCH_SIZE,1,h_zoom, w_zoom)
        mask_flow = mask_flow*(1.-((pre_mask>specials.OFF_THRESH).long()==target_2Dr.long()).float())

        # testing/input
        self.smap3x3.vtestcase.testbot_input(mask_flow.reshape(BATCH_SIZE,C_zoom,3*3,h_zoom, w_zoom), filename="_x.npy", dim=4)
        self.smap3x3.vtestcase.testbot_input(coord_flow.reshape(BATCH_SIZE,C_zoom,3*3,h_zoom, w_zoom), filename="_y.npy", dim=4)
        ...
        weights = pre_weights - (1.-(pre_weights>specials.OFF_THRESH).float().detach())*(torch.zeros_like(pre_mask)+pre_weights)
        ...
        #######################
        self.smap3x3.vtestcase.testbot_input(weights.reshape(BATCH_SIZE,C_zoom,3*3,h_zoom, w_zoom), filename="_out.npy", dim=4)
...
class DEPRectify(DefaultRectify):
    ...
    def compute_pre_allow_matrices(self, weight, target_repr):
        ...
        s = torch.sum((pre_allow==1.).float(),dim=1,keepdim=True)
        pre_allow = utils.agg(utils.flip(weight*s,2).reshape(BATCH_SIZE,-1,3,3,1,height, width)).reshape(BATCH_SIZE,-1,3*3,1,1,height, width)
        pre_allow_s = utils.agg(utils.flip(torch.zeros_like(weight)+s,2).reshape(BATCH_SIZE,-1,3,3,1,height, width)).reshape(BATCH_SIZE,-1,3*3,1,1,height, width)
    ...
    def prepare_flows_for_mask(self, pre_allow, pre_allow_s, weights_b, target_repr):
        ...
        pre_allow = torch.max(pre_allow,dim=2,keepdim=True).values
        ...
        y_flow = torch.where(y_flow==0., 1.-y_flow, 0.*y_flow)
        y_flow = torch.where(y_flow==0., y_flow, pre_allow_s)
    ...
    def rectificate_flow(self, new_x_z_mask_value, pre_x, pre_y, pre_z, pre_mask, panels, target, original_size):
        ...
        k_d = torch.abs(pre_z)
        k_t = torch.max(torch.abs(torch.cat([pre_x, pre_y],dim=-3)),dim=-3,keepdim=True).values
        k_d = torch.where((k_d/(k_t+1.))>1., 1e-1/((k_d+1e-7)**(1-1e-5)/(k_d+1e-7)**(-1e-5))*(k_d**2), 3e-1*(k_d+1.)**(4)/((k_d+1.)**(6)+1.)*k_d)
        k_t = k_d.detach()/(pre_z+torch.exp(-1e7*pre_z.detach()))
        ...
        pre_mask = pre_mask.reshape(BATCH_SIZE,-1,3,3,1,height, width)
        ...
        # testing/in
        pos_key_query_grdf = self.smap3x3.vtestcase.testbot_in(pos_key_query_grdf.reshape(BATCH_SIZE,C_zoom*3*3,height, width))
        ...
        pre_mask = (pre_mask).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width)
        pre_mask = utils.agg(pre_mask).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width)
        weight = utils.agg((new_x_z_mask_value[:,:,:,:,-1:,:,:]).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width))
        ...
        # testing/out
        pos_key_query_grdf = self.smap3x3.vtestcase.testbot_out(pos_key_query_grdf)
        ...
        # testing/input
        self.smap3x3.vtestcase.testbot_input(n_flow.reshape(BATCH_SIZE,C_zoom,3*3,h_zoom, w_zoom), filename="_x_flow.npy", dim=4)
...
class FUTRectify(DefaultRectify):
    ...
    def compute_pre_allow_matrices(self, weight, target_repr):
        ...
        pre_allow = specials.INF*torch.ones_like(weight)
        pre_allow[:,:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)] = (weight[:,:,:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)])*target_repr.reshape(BATCH_SIZE,-1,1,1,h_zoom, w_zoom)
        s = torch.sum((pre_allow==1.).float(),dim=2,keepdim=True)
        pre_allow = utils.agg(utils.flip(weight*s,2).reshape(BATCH_SIZE,-1,3,3,1,height, width)).reshape(BATCH_SIZE,-1,3*3,1,1,height, width)
        pre_allow_s = utils.agg(utils.flip(torch.zeros_like(weight)+s,2).reshape(BATCH_SIZE,-1,3,3,1,height, width)).reshape(BATCH_SIZE,-1,3*3,1,1,height, width)
        return pre_allow.detach(), pre_allow_s.detach()
    ...
    def prepare_flows_for_mask(self, pre_allow, weights_b, target_repr):
        ...
        return torch.where(n_flow==0.,-torch.ones_like(n_flow),n_flow)
    ...
    def prepare_flows_for_coord(self, pre_allow, weights_b, target_repr):
        ...
        y_flow = (y_flow.reshape(BATCH_SIZE,-1,height, width)[:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)]).reshape(BATCH_SIZE,-1,h_zoom, w_zoom)
        coord_flow = (utils.agg(allow).reshape(BATCH_SIZE,-1,height, width)[:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)]).reshape(BATCH_SIZE,-1,h_zoom, w_zoom)
        return y_flow.detach(), coord_flow.detach()
    ...
    def rectificate_flow(self, new_x_z_mask_value, pre_x, pre_y, pre_z, pre_mask, panels, target, original_size):
        ...
        # self.smap3x3.vtestcase.testbot_input((pre_mask.reshape(BATCH_SIZE,C_zoom*3*3,height, width)[:,:,((height-h_zoom)//2):((height+h_zoom)//2),((width-w_zoom)//2):((width+w_zoom)//2)]).reshape(BATCH_SIZE,C_zoom,3*3,h_zoom, w_zoom)[:,:,4,:,:])
        pre_mask = (pre_mask).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width)
        pre_mask = utils.agg(pre_mask).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width)
        weight = utils.agg((new_x_z_mask_value[:,:,:,:,-1:,:,:]).reshape(BATCH_SIZE,C_zoom,3,3,1,height, width))
        ...
        n_flow = self.prepare_flows_for_mask(pre_allow_s, (weights_b>specials.OFF_THRESH).detach().float(), target_2Dr)
        y_flow, coord_flow = self.prepare_flows_for_coord(pre_allow, (weights_b>specials.OFF_THRESH).detach().float(), target_2Dr)
