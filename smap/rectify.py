...
class DEPRectify(DefaultRectify):
    ...
    def compute_pre_allow_matrices(self, weight, target_repr):
        ...
        pre_allow_s = utils.agg(utils.flip(pre_allow_s,2).reshape(BATCH_SIZE,-1,3,3,1,height, width)).reshape(BATCH_SIZE,-1,3*3,1,1,height, width)

        pre_allow = utils.agg(utils.flip(pre_allow,2).reshape(BATCH_SIZE,-1,3,3,1,height, width)).reshape(BATCH_SIZE,-1,3*3,1,1,height, width)
    ...
    def rectificate_flow(self, new_x_z_mask_value, pre_x, pre_y, pre_z, pre_mask, panels, target, original_size):
        ...
        neg_pre_key_query = utils.calculate_key_query(pre_x*k_t.detach(), pre_y*k_t.detach(), (pre_z*k_t).detach(), panels, (self.smap3x3.window_h, self.smap3x3.window_w), original_size, self.smap3x3.camera_matrix_inv, device=self.smap3x3.device)
        pos_pre_key_query = utils.calculate_key_query(pre_x*k_t, pre_y*k_t, (pre_z*k_t), panels, (self.smap3x3.window_h, self.smap3x3.window_w), original_size, self.smap3x3.camera_matrix_inv, device=self.smap3x3.device)
