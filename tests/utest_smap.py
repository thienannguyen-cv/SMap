...
class SMapUTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.smap = SMap(self.img_shape[0], self.img_shape[1], self.camera, self.device, n=self.n).to(self.device)
    ...
    def test_prepare_flows_for_mask(self):
        ...
        if case=="blocked":
            ...
            try:
                if offsetx!=target_offsetx or offsety!=target_offsety:
                    assert torch.sum((actual[0,offset,:,:]>0).float()).reshape(-1).cpu().numpy()[0] == ((self.img_shape[0]-2)*(self.img_shape[1]-2)), "Flows preparing test failed. "
                else:
                    assert torch.sum((actual[0,offset,:,:]>0).float()).reshape(-1).cpu().numpy()[0] == (((self.img_shape[0]-2)*(self.img_shape[1]-2)-1) if is_activated else ((self.img_shape[0]-2)*(self.img_shape[1]-2))), "Flows preparing test failed. "
            except Exception as e:
                print(f"is_activated: {is_activated}")
    ...
    def test_SMap_forward(self):
        ...
        actual = self.smap.calculate_weights(actual, self.img_shape)
